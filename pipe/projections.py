...
#If BaseExcess <= 0 then either HC03<=10 or Lactate >=6
    ...
    for var in proj_variables:             
        data_copy['{}-phys-dist'.format(var)] = [np.round(sum(np.square(np.array([x[var][i][t].x - \
                    normalize(data.loc[i,'{}-{}'.format(var,t)], var,ranges, norm_to = 'physical') for t in range(6)]))),4) for i in range(k)]
    print("Data diff")
...
#%%
def projection_model_physical3(data2,ranges):
...
def project():
    ...
    for i, data_ in enumerate(data_split):
        ...
        final_phy_3,val_phy3 = projection_model_physical3(data_.copy(),ranges )
        print("Done to physical 3")
        ...
        final_normal_3,val_normal3 = projection_model_normal(final_phy_3.copy(),ranges)
        print("Done to Normal 3")
...
if __name__ == '__main__':
    ...
    physical3, normal3 = project()
    
    physical3.to_csv('physical_30oct.csv')
    normal3.to_csv('normal_30oct.csv')
