# -*- coding: utf-8 -*-
"""
Created on Thu Nov 25 22:45:45 2021
...
def modelfit_svm(
    X_train, y_train,
    X_test, y_test,
    param, param_tune, wts = None
):
    
    
    #X_1, X_2, y_1, y_2 = train_test_split(X_train, y_train, test_size=0.33, random_state=42)
    ...
    grid = GridSearchCV(pipe, param_tune,  n_jobs = -1, verbose = 3, cv = 2)
    grid.fit(X_train, y_train)
    
    print(grid.best_params_)
    
    svm_final = SVC( C = grid.best_params_['model__C'], gamma = grid.best_params_['model__gamma'], kernel = 'rbf', probability = True, class_weight='balanced')
    pipe = Pipeline([ ('model' , svm_final)])
